    ...
    switch(erro){
        ...
        case ERRO_DE_SINTAXE:
            if (modo_binario) {
                ...
                putsUSART((char *)err_stx);
            }
            break;
         case SPI_TIMEOUT:
            if (modo_binario) {
                initCommand(3);
        		appendMsg(sprintf(getOutbfPtr(),"SPI Timeout"));
        		sendCommand ();
            }
            else{
                putsrom "\r\n SPI Timeout");
...
/**
 * Informa se estamos em modo binario
 */
BYTE isBinary(){ return modo_binario;}
...
void inputUSART(){
	...
	inputbuffer[inputbuffer_ptr] = ReadUSART();	
	inputbuffer_ptr++;
...
BYTE dispatch (){
    ...
    if (!strcmppgm2ram(param[0], "SCAN")) {
//        initCommand(3);
//        appendMsg(sprintf(getOutbfPtr(), "Enviando Scan...\r\n", 32 ));
//        sendCommand ();      
        // Solicite o scan do R12 via SPI
        //enableScanLoad();
        //loadScan();
        loadBatch();
        // Solicite o envio do scan para o Host
        enableScanSend(TRUE);
        envie_retorno = FALSE;
        return TRUE;   
    }
    ...
    // Envia tokem para a SPI
    if (!strcmppgm2ram(param[0], "R12")) {
        if (!strcmppgm2ram(param[1], "SCAN")) {
        	loadScan();	
            return TRUE;         
        }
        putsSPIM(param[1]);
        ...
        initCommand(3);
        appendMsg(sprintf(getOutbfPtr(), "Enviando SPI %s\r\n", param[1]));
        sendCommand ();
        return TRUE;
    }
    
   
    // Mostra status das flags de habilitacao e outros quetais
    if (!strcmppgm2ram(param[0], "SET")) {
        CRLF;
        if (!strcmppgm2ram(param[1], "POUT")) {
        	LPT_POUT = atoi(param[2]);
            printf ("Paper out = %u \r\n", LPT_POUT);
            //LATCbits.LATC2 ^= 1;
            return TRUE;         
        }	
	...
	if (!strcmppgm2ram(param[0], "RESET")) {
        CRLF;
        if (!strcmppgm2ram(param[1], "LPT")) {
        	LPT_ACK = 0;
			Delay100TCYx(10);
			LPT_ACK = 1;
            return TRUE;
